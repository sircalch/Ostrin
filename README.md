...
The compiler suite currently passes **90 integration tests**. Function calls
...
`Result` and `?` still only run through the interpreter; `Option<T>` (Some/None, match, `find`) compiles natively. The runtime is still

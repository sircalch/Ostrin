...
The compiler suite currently passes **140 integration tests and 6 differential
...
WASI C toolchain, checksums, and a program smoke test. The cooperative runtime avoids
thread-only headers unless `--native-threads` is requested. This is still a WASI distribution
artifact, not yet a browser backend for Ostrin programs.

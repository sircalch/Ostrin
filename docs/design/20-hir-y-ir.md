...
payload escalar también atraviesan ya el emisor IR, incluidos sus marcadores de ownership;
el backend mantiene HIR/AST como fallback verificado para `Option` gestionado o con patrones,
...
| Emisor IR | `ir_c.rs` | Genera C desde SSA/CFG para funciones escalares, `String`, `List<T>` escalar, operaciones hash escalares de `Map`/`Set`, `Option<T>` escalar, ramas, bucles, `phi` y enteros de ancho fijo comprobados; emite ownership para las familias migradas y deja fallback seguro para lo demás |
...
4. **RC + último uso** sobre el IR (`--leak-check`: los ejemplos deben terminar sin objetos vivos).
   ...
   tipados y retienen sus campos. `Option` escalar se copia por valor; la IR aún deja barreras
   explícitas para `Option` gestionado o con patrones, llamadas que transfieren ownership,
   loops, scopes anidados y escapes.

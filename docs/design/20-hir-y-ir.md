...
1. **HIR + verificador + `--hir`** para *todo* lo que el checker tipa; medida de cobertura por ejemplo (ratchet).
   ...
   La prueba diferencial conserva el baseline actual de fallback (1 418 funciones agregadas
   sobre los ejemplos); el incremento acotado viene de `std.viz.boxplot` y `std.viz.table`, que
   quedan pendientes de migrar a IR, y solo permite reducirlo o justificar explícitamente otro
   aumento.

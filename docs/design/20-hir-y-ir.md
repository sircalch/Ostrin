...
con elementos escalares y las operaciones escalares de `Map<K,V>`/`Set<T>` también atraviesan
ya el emisor IR, incluidos sus marcadores de ownership; el backend mantiene HIR/AST como
fallback verificado para lookups con `Option`, iteradores, agregados complejos y escapes
...
| Emisor IR | `ir_c.rs` | Genera C desde SSA/CFG para funciones escalares, `String`, `List<T>` escalar, operaciones hash escalares de `Map`/`Set`, ramas, bucles, `phi` y enteros de ancho fijo comprobados; emite ownership para las familias migradas y deja fallback seguro para lo demás |
...
4. **RC + último uso** sobre el IR (`--leak-check`: los ejemplos deben terminar sin objetos vivos).
   ...
   tipados y retienen sus campos. La IR aún deja barreras explícitas para lookups con `Option`,
   llamadas que transfieren ownership, loops, scopes anidados y escapes.

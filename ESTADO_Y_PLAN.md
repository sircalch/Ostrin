...
- `List/Map/Set/Option/Result`, combinadores con lambdas expandidas en línea, `try` y `try catch`
  inline para `Result` escalar con error `String`.
...
- `Option`/`Result` estructurales también se generan desde HIR: `Some`/`None`, `Ok`/`Err`,
  ...
  combinadores `Result.map`, `Result.map_err` y `Result.then` con lambda inline también usan
  IR; los combinadores de `Option`, handlers no inline y payloads compuestos siguen usando el
  fallback AST.
...
| IR de bloques | HIR→CFG disponible con `--ir`; el emisor C cubre ramas, bucles escalares con `phi`, `String` (incluidos `split/lines`), `Result` escalar de parseo con `match`/consultas, `try`, `try catch` inline y `map`/`map_err`/`then` de `Result`, records concretos con campos anidados, `Option<Record>`, listas escalares/String, operaciones hash escalares y `Option` escalar/String con patrones `Some/None`; `for`/iteradores, combinadores de `Option`, handlers no inline, patrones anidados y colecciones complejas aún no reemplazan el backend C completo |

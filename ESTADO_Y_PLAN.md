...
*Corte: 2026-09-20 · rama `main` · 6 pruebas diferenciales y 152 de integración en verde.*

Este documento resume **qué existe hoy**, **qué no**, y **por dónde se puede avanzar**.
Para la historia detallada, ver `CONTEXTO_PROYECTO.md` (secciones 1–185); para el diseño
...
| HIR/IR | `hir.rs`, `hir_c.rs`, `ir.rs`, `ir_c.rs` | HIR verificado, CFG con temporales explícitos y emisor C desde IR para escalares, `String`, listas escalares, enteros de ancho fijo y control de flujo, con fallback HIR/AST acotado |
...
`String` y `List<T>` también se consumen al generar C; agregados, iteradores u operaciones todavía no
...
gestionada migrada a la IR es `String`, seguida por el núcleo escalar de `List<T>`: literales,
concatenación, igualdad, llamadas, ramas con `phi`, indexación, mutación, `print` y marcadores
...
| Memoria en nativo | Registro, destructores tipados para records/colecciones y entornos de tareas, `clone`/`drop`, cleanup automático de locales directos, bloques anidados, ramas, loops y cancelación de tareas en AST/HIR, y `--leak-check`; `String` y listas escalares ya consumen ownership desde IR, pero ARC completa de agregados sigue pendiente |
| IR de bloques | HIR→CFG disponible con `--ir`; el emisor C cubre ramas, bucles escalares con `phi`, `String` y el núcleo `List<T>` escalar; `for`/iteradores, mapas/sets, records y colecciones complejas aún no reemplazan el backend C completo |
| Ownership/último uso | `--ownership-report`, `--ownership-check` y `--ownership-ir`; la IR ya inserta y consume retain/release lineal para `String` y listas escalares, mientras llamadas transferentes, agregados complejos, loops, scopes y escapes siguen conservadores |
...
```powershell
cd compiler
cargo test                                   # 6 diferenciales + 152 de integración

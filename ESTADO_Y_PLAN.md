...
| HIR/IR | `hir.rs`, `hir_c.rs`, `ir.rs`, `ir_c.rs` | HIR verificado, CFG con temporales explícitos y emisor C desde IR para escalares, `String`, `Result<Int, String>`/`Result<Float, String>` con `try`, `try catch` inline y handlers globales, records concretos, `Option<Record>`, listas escalares, mapas/conjuntos escalares, enteros de ancho fijo y control de flujo, con fallback HIR/AST acotado |
...
del tipo envolvente; payloads compuestos de `Result` siguen en HIR/AST hasta tener el mismo
contrato.

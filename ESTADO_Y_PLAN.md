...
Para la historia detallada, ver `CONTEXTO_PROYECTO.md` (secciones 1–227); para el diseño
...
| HIR/IR | `hir.rs`, `hir_c.rs`, `ir.rs`, `ir_c.rs` | HIR verificado, CFG con temporales explícitos y emisor C para escalares, `String`, `Result<Int, String>`/`Result<Float, String>` con `try`, `try catch` inline, handlers globales y aliases locales de handlers globales, wrappers `Option`/`Result` sobre `List`, `Map` y `Set` con payload gestionado, records concretos, iteradores de records concretos (`next() -> Option<T>`), iteración de canales mediante `receive() -> Option<T>`, `spawn {}` lineal sin capturas y `Task.join()`, `Option<Record>`, listas escalares, mapas/conjuntos escalares, enteros de ancho fijo y control de flujo, con fallback HIR/AST acotado |
...
La primera tarea que cruza completamente HIR→IR→C es `spawn {}` sin capturas y de cuerpo lineal,
con `Task.join()` cooperativo o sobre hilos nativos; capturas, control de flujo interno y
`spawn_scope` conservan el fallback verificado hasta que exista una ABI de entorno y cancelación
equivalente en la IR.
...
Agregados complejos, iteradores propios genéricos o indirectos, capturas/control de flujo dentro de `spawn`,

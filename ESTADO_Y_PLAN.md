...
Para la historia detallada, ver `CONTEXTO_PROYECTO.md` (secciones 1–228); para el diseño
...
La primera tarea que cruza completamente HIR→IR→C es `spawn {}` de cuerpo lineal, incluyendo
capturas inmutables por valor mediante un entorno C con retain/release, y `Task.join()` cooperativo
o sobre hilos nativos; control de flujo interno, tareas anidadas y `spawn_scope` conservan el
fallback verificado hasta que exista una ABI de CFG, entorno y cancelación equivalente en la IR.
...
Agregados complejos, iteradores propios genéricos o indirectos, control de flujo o tareas anidadas dentro de `spawn`,
